...
def test_optimal_operation_without_flex_initialization(hydrogen_plant):
    ...
    assert hydrogen_plant.opt_power_requirement is not None, "opt_power_requirement should be populated"
    ...
    total_power_input = sum(
        instance.total_power_input[t].value for t in instance.time_steps if instance.total_power_input[t].value is not None
    )
    ...
    for t in instance.time_steps:
        ...
        hydrogen_demand = instance.hydrogen_demand[t].value if hasattr(instance.hydrogen_demand[t], 'value') else instance.hydrogen_demand[t]
        storage_charge = instance.dsm_blocks["h2_seasonal_storage"].charge[t].value
        storage_discharge = instance.dsm_blocks["h2_seasonal_storage"].discharge[t].value
        
        # Check that the balance holds within a small tolerance
        balance_difference = abs(electrolyser_out - (hydrogen_demand + storage_charge - storage_discharge))
        assert balance_difference < 1e-3, f"Hydrogen balance mismatch at time {t}, difference: {balance_difference}"
...
def test_ramping_constraints_without_flex(hydrogen_plant):
    ...
    for t in list(instance.time_steps)[1:]:
        ...
        discharge_prev = instance.dsm_blocks["h2_seasonal_storage"].discharge[t - 1].value
        ...
        if charge_prev is not None and charge_curr is not None:
            assert abs(charge_curr - charge_prev) <= ramp_up, f"Charge ramp-up at time {t} exceeds limit"
        if discharge_prev is not None and discharge_curr is not None:
            assert abs(discharge_curr - discharge_prev) <= ramp_down, f"Discharge ramp-down at time {t} exceeds limit"

...
def test_pay_as_bid_demand_exceeds_supply_completes_orders():
    """
    Regression: when demand exceeds the offered supply (e.g. a reserve shortfall),
    PayAsBidRole must still return every accepted and rejected order with both
    ``accepted_volume`` and ``accepted_price`` set. Previously a demand order left
    unserved after supply was exhausted got a bogus positive ``accepted_volume`` with
    no ``accepted_price``, which crashed ``set_dispatch_plan`` downstream
    (``KeyError: 'accepted_price'``).
    """
    config = MarketConfig(
    ...
    # no phantom demand: accepted demand exactly matches the 500 MW that could be served

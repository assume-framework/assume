...
@pytest.mark.parametrize("dummy_generation", "dummy_demand", "expected_redispatch",
                         [
                             (pd.Series([0, 3000, 0]), 3000, pd.Series([0, 0])), # all demand covered by coal_S
                             (pd.Series([3000, 0, 0]), 3000, pd.Series([-1000, 1000])), # downwards redispatch of wind_N, upward redispatch of coal_S
                            (pd.Series([0, 0, 0]), 3000, pd.Series([2000, 1000])), # zonal market does not cover demand -> find cheapest solution
                         ])

@pytest.mark.require_network
def test_two_nodes_redispatch(dummy_generation, dummy_demand, expected_redispatch):
    market_config = make_market_config(simple_redispatch_market_config)
    market_config.param_dict["grid_data"] = grid_data
    market_config.param_dict["log_flows"] = True
    assert len(products) == 1
    orderbook = []
    for _ in range(len(dummy_generation)):
        supply_bid = order.copy()
        supply_bid["unit_id"] = generators.index[_]
        supply_bid["volume"] = dummy_generation[_]
        supply_bid["price"] = generators.marginal_cost[_]
        supply_bid["node"] = generators.node[_]
        supply_bid["bid_id"] = generators.index[_] + "_bid"
        orderbook.append(supply_bid)
    demand_bid = order.copy()
    demand_bid["unit_id"] = demand.index[0]
    demand_bid["volume"] = dummy_demand
    demand_bid["price"] = 1000.0
    demand_bid["node"] = demand.node[0]
    demand_bid["bid_id"] = demand.index[0] + "_bid"

...
class DSMFlex:
    ...
    def _export_bus_depot_graphs_to_csv(self, instance, evs, charging_stations, time_steps):
        ...
        if ev_charge_data or ev_discharge_data or ev_usage_data:
            ...
            # Add PV used data if PV is present
            if hasattr(instance, 'pv_used'):
                pv_used_data = [pyo.value(instance.pv_used[t]) for t in time_steps]
                ev_power_data['pv_used'] = pv_used_data
    ...
    def _print_bus_depot_summary(self, instance, evs, charging_stations, time_steps):
        ...
        # Export detailed energy flow analysis
        try:
            self.export_detailed_energy_flow(instance, save_path="detailed_energy_flow.csv")
        except Exception as e:
            print(f"Warning: Could not export detailed energy flow: {e}")
        """
    ...
    def export_detailed_energy_flow(self, instance, save_path="detailed_energy_flow.csv"):
        ...
        print(f"  Total Power Output:       {df['total_power_output_kW'].sum():.2f} kWh")
        ...
        # Verify formula
        print("\nFormula Verification (first 3 timesteps):")
        print("  total_power_output = CS charge + PV surplus")
        for i in range(min(3, len(time_steps))):
            cs_charge = data['total_cs_charge_kW'][i]
            pv_surplus = data['pv_surplus_kW'][i]
            total_output = data['total_power_output_kW'][i]
            calculated = cs_charge + pv_surplus
            match = "OK" if abs(calculated - total_output) < 0.0001 else "MISMATCH"
            print(f"    t={i}: {cs_charge:.4f} + {pv_surplus:.4f} = {calculated:.4f} vs {total_output:.4f} [{match}]")

...
def load_pypsa(
    ...
    for _, generator in network.generators.iterrows():
        ...
        world.add_unit(
            ...
            "powerplant_operator",
            {
                "min_power": max_power * generator.p_min_pu if generator.p_min_pu is not None else 0,
                "max_power": max_power,
    ...
    for _, storage in network.storage_units.iterrows():
        ...
        unit_type = "storage"
        max_power_charge = storage.p_nom * storage.p_min_pu if storage.p_min_pu is not None else storage.p_nom
        max_power_discharge = storage.p_nom * storage.p_max_pu if storage.p_max_pu is not None else storage.p_nom

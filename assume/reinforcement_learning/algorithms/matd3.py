...
# Diagnostic: sweep of the first action dimension used to print the critic's
# Q-landscape after every policy update. Grid runs over the actor's full
# softsign output range [-1, 1] with a step size of 0.1 (21 points).
CRITIC_PROBE_STEP = 0.1
CRITIC_PROBE_ACTIONS = [
    round(-1.0 + i * CRITIC_PROBE_STEP, 2) 
    for i in range(int(2.0 / CRITIC_PROBE_STEP) + 1)
]
    ...
    def update_policy(self):
        ...
        for step in range(self.learning_config.gradient_steps):
            ...
            for i, strategy in enumerate(strategies):
                ...
                # Debug prints, commented out for run 13: the th.rand(1) draw
                # shifts the exploration/batch stream (HANDOFF caveat), and the
                # 20 extra forward passes fire on most steps in the multi-agent
                # inc-dec case, where target_Q_values is often positive.
                # if th.rand(1) > 0.9:

...
def determine_flexibility(reference_curve, tolerance):
    ...
    for technology, curve_values in reference_curve.items():
        ...
        min_flexibility[technology] = {
            ...
            'min_positive': [max(0, value + tolerance) for value in curve_values]  # Positive flexibility
        }
...
def calculate_deviation(reference_state, min_flexibility, max_flexibility, objective_function):
    deviations = {}
...
def organize_results(deviations, min_flexibility, max_flexibility):
    ...
    pos_flex_df = pd.DataFrame(columns=['Technology', 'Time Step', 'Flexibility Capacity', 'Power Output Adjustment'])
    neg_flex_df = pd.DataFrame(columns=['Technology', 'Time Step', 'Flexibility Capacity', 'Power Output Adjustment'])
...
def calculate_deviation(reference_costs, reference_profit, flexibility_costs, flexibility_profit):
...
def construct_power_input_dataframes(reference_power_input, flexibility_power_input):
    ...
    reference_df = pd.DataFrame({'Time Step': reference_series.index, 'Reference Power Input': reference_series.values})
    flexibility_df = pd.DataFrame({'Time Step': flexibility_series.index, 'Flexibility Power Input': flexibility_series.values})
...
def integrate_flexibility_measures(current_state, positive_flexibility_capacity, negative_flexibility_capacity, flexibility_objective):
    updated_state = {}
...
def generate_power_input_dataframes(reference_state, updated_state):
    ...
    reference_df.columns = [f'Reference_{tech}_Power_Input' for tech in reference_df.columns]

...
class ReplayBuffer():
    ...
    def __init__(self,
        ...
        self.observations = np.zeros((self.buffer_size, self.n_rl_agents, self.obs_dim), dtype = self.np_float_type)
        self.next_observations = np.zeros((self.buffer_size, self.n_rl_agents, self.obs_dim), dtype = self.np_float_type)
        
        self.actions = np.zeros((self.buffer_size, self.n_rl_agents, self.act_dim), dtype = self.np_float_type)
        self.rewards = np.zeros((self.buffer_size, self.n_rl_agents), dtype = self.np_float_type)
        ...
        if psutil is not None:
            ...
            total_memory_usage = self.observations.nbytes + self.actions.nbytes + \
                self.rewards.nbytes + self.next_observations.nbytes
    ...
    def add(self, obs, actions, reward):
        ...
        self.next_observations[self.pos] = np.array(obs[1], dtype = self.np_float_type).copy()
    ...
    def save_params(self, simulation_id):
        ...
        params = {'observations': self.observations[:self.pos],
    ...
    def load_params(self, simulation_id):
        ...
        directory = 'output/' + simulation_id + '/buffer/'
        if not os.path.exists(directory):
            raise FileNotFoundError('Specified directory for loading the buffer does not exist!')

        self.pos = load_obj(directory, 'pos')
        
        self.observations[:self.pos] = load_obj(directory, 'observations')[:self.pos]
        self.next_observations[:self.pos] = load_obj(directory, 'next_observations')[:self.pos]

...
class WriteOutput(Role):
    ...
    def get_sum_regret(self):
        query = text(
            f"select unit, SUM(regret) FROM rl_params where simulation='{self.simulation_id}' GROUP BY unit"
        )

        with self.db.begin() as db:
            regrets_by_unit = db.execute(query).fetchall()

        avg_regret = (
            sum(r[1] for r in regrets_by_unit) / len(regrets_by_unit)
            if len(regrets_by_unit)
            else 0
        )

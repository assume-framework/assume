...
class CsvForecaster(Forecaster):
    ...
    def calculate_node_specific_congestion_forecast(self) -> pd.DataFrame:
        ...
        try:
            self.lines["bus0"] = self.lines["bus0"].astype(
                self.demand_units["node"].dtype
            )
            self.lines["bus1"] = self.lines["bus1"].astype(
                self.demand_units["node"].dtype
            )
        except Exception:
        ...
        for line_id, line_data in self.lines.iterrows():
            ...
            if load1 is None:
                self.logger.warning(
                    f"Missing net-load for node {node1}; assuming zero-series"
                )
            ...
            if load2 is None:
                self.logger.warning(
                    f"Missing net-load for node {node2}; assuming zero-series"
                )

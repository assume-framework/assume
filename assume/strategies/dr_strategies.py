...
class RealTimeTariffDR:
    ...
    def add_real_time_tariffs(self, model):
        model.model.real_time_price = Param(model.model.time_steps, initialize=self.real_time_prices)

    def update_objective_function(self, model):
        def _objective_rule(m):
            return sum(m.real_time_price[t] * (sum(m.power_hp[i, t] for i in m.heat_pumps)) for t in m.time_steps)
                                               # +

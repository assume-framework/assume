...
class PortfolioRLStrategy(TorchLearningStrategy, UnitOperatorStrategy):
    ...
    **kwargs : Arbitrary keyword arguments.
    ...
    def __init__(self, *args, **kwargs):
        ...
        obs_dim = kwargs.pop("obs_dim", 3* self.foresight + unique_obs_dim) # e.g. 36 shared observations + 4 unique_observations  
    ...
    def calculate_bids(
        ...
        **kwargs,
        ...
        units by their marginal cost, bidding their inflexible generation at 0 €/MWh and iteratively 
    ...
    def get_actions(self, next_observation:th.Tensor) -> tuple[th.Tensor, th.Tensor]:
        ...
        if self.learning_mode and not self.evaluation_mode:
            ...
            if self.collect_initial_experience_mode:
                freq = 10
                curr_action = th.randint_like(curr_action, low=-1*freq, high=1*freq+1) / freq 
    ...
    def get_individual_observations(
        ...
        cost_bins = np.quantile(flex_cost, q=bins, weights=flex_quant, method='inverted_cdf')
        ...
        for product, product_orders in groupby(orderbook, market_getter):
            ...
            for order in product_orders:
                ...
                if not accepted_volume and marginal_cost > market_clearing_price:
                    ...
                    opportunity_cost+= -1 * (market_clearing_price - marginal_cost) * order["volume"] * duration
                    reward = -1
                
                if order["price"] == market_clearing_price:
                    price_forecast = unit.forecaster.price[marketconfig.market_id][prod_start]
                    price_diff = (market_clearing_price - price_forecast) * accepted_volume

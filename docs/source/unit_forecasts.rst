...
1. :code:`preprocess`: this method prepares information / forecasts / etc. for steps 2. and 3.
2. :code:`initialize`: this method initializes all forecast timeseries, after all units are created
3. :code:`update`: this method is used to change forecasts during runtime (shall be called during calculate bids of bidding strategies)

Generally, those three methods call functions that can be specified in the config.yaml or the unit csv files.
In the config.yaml they have to be specified at :code:`forecast_algorithms` as :code:`{prefix}_{forecast_metric}`: :code:`algorithm_id`.
`prefix` denotes which of the three methods :code:`preprocess`, :code:`update`, :code:`initialize` is meant (:code:`initialize` takes no prefix).
...
For specification in the unit csv files the mapping is done with another fixed :code:`forecast_*` in the beginning:
:code:`forecast_{prefix}_{forecast_metric}` as column name and :code:`algorithm_id` as value for the corresponding unit.
...
2. Forecast series can be directly given to the forecasters on instantiation when creating a simulation by hand (i.e. not using specific scenario loaders). Note: the forecasting_algorithms should be set to :code:`{forecast_metric}_keep_given` for all forecasting_metrics that use this way! Otherwise the forecast will get overwritten.
...
1. Add a new forecaster for the custom unit type by creating a new class that inherits from :code:`UnitForecaster` and implements the necessary fields and methods.
2. Use the :code:`CustomUnitForecaster`, which allows for flexible definition of forecast fields without creating a new class.

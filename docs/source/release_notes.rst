...
**Bug Fixes:**
  ...
  - **Fix stale storage SoC when bids are filtered out** (`#562 <https://github.com/assume-framework/assume/issues/562>`_, `#837 <https://github.com/assume-framework/assume/issues/837>`_): The SoC used to be propagated only over the time steps a storage actually had orders in, so a gap in the orderbook - produced for instance by ``remove_empty_bids`` - left the pre-filled ``initial_soc`` in place for the rest of the horizon, and the next market opening bid against that stale value. A storage now tracks how far its SoC has been propagated: committing energy invalidates the SoC from that time step on, and reading it propagates as far as needed. Propagation is also the single place where the committed energy is made feasible - clipped first to the unit's power limits and then to what the SoC allows - so that volumes stacked into one time step by several markets can no longer add up beyond what the unit can run at. Read the SoC through the new ``SupportsMinMaxCharge.get_soc`` (and set a known one with ``set_soc``) - indexing ``outputs['soc']`` directly can return the pre-filled value. No public signatures changed.

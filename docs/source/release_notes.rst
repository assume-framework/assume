...
**New Features:**
  ...
  - **Grid-fee (tariff) market**: A dynamic network tariff can now be published as an ordinary ``pay_as_clear`` market with a non-energy ``product_type``, opening ahead of the EOM. ``GridFeeDsoStrategy`` (registered as ``powerplant_grid_fee_dso``) lets a DSO unit submit the tariff as a stepwise supply curve - a flat fee, a time-of-use adder, or a headroom block plus a scarcity step whose clearing price is a function of the announced aggregate withdrawal. ``GridFeeAnnouncementStrategy`` (``household_grid_fee_announcement`` / ``industry_grid_fee_announcement``) submits the price-taking demand bid that announces a unit's planned withdrawal and makes the cleared fee reach ``outputs["grid_fee_accepted_price"]``. The ``price_plus_grid_fee`` forecast update algorithm folds the published fee into the unit's electricity price forecast, so it enters a rolling-horizon DSM optimisation objective. See ``examples/inputs/example_lv_tariff``.
...
**Improvements:**
  ...
  - **Expose the provisional rolling-horizon plan**: ``DSMFlex`` units now also publish ``planned_power_requirement``, the schedule over the whole look-ahead window rather than only the committed prefix. ``opt_power_requirement`` keeps its previous meaning (committed schedule only); the new series lets strategies announce a position further ahead than the unit commits, which is what a market delivering ahead of the EOM needs.

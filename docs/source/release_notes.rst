...
**Bug Fixes:**
  ...
  - **Fix stale storage SoC when bids are filtered out** (`#562 <https://github.com/assume-framework/assume/issues/562>`_, `#837 <https://github.com/assume-framework/assume/issues/837>`_): The SoC used to be propagated only over the time steps a storage actually had orders in, so a gap in the orderbook - produced for instance by ``remove_empty_bids`` - left ``initial_soc`` in place for the rest of the delivery period, and the next market opening bid against that stale value. ``ClearingMessage`` now carries the cleared delivery period, ``UnitsOperator.set_unit_dispatch`` notifies every unit it manages (not only those present in the orderbook), and ``SupportsMinMaxCharge.update_soc`` propagates the SoC densely over that period. ``BaseUnit.set_dispatch_plan`` and ``UnitsOperator.set_unit_dispatch`` therefore take additional ``start_time``/``end_time`` arguments.

...
### 14b / 14c — the same films, re-recorded twice

**14b — regime-stratified probes.** `OBS_REGIMES=1` makes the recorder rank the
buffer on the observation's scaled residual load and draw `--n-obs` probes
*inside each regime band*, so the film can be read separately per Nash
equilibrium. Bands found: `sb02a` idle+pivotal (8 probes), `sb02b` all three
(12), `sb02c` idle+bertrand (8) — all three seeds agreeing per case, and
`sb02b` is therefore the only case containing both equilibria.

**14c — plus the stored-action sweep (workstream A.1).** Same again with both
sweeps recorded. This is the batch findings 15 and 16 come from.

```bash
RUN_NAME=14b-eom-regimes OBS_REGIMES=1 RERUN=1 bash .../cluster/eom_critic_evolution.sh
RUN_NAME=14c-eom-stored  OBS_REGIMES=1 RERUN=1 bash .../cluster/eom_critic_evolution.sh
```

- data: `runs/data/14b-eom-regimes/`, `runs/data/14c-eom-stored/`
- img: `runs/img/14b/`, `runs/img/14c/` — figures now default to
  `runs/img/<leading number of --data-dir>`, so a batch lands in its own folder
...
**Reading — see findings 15 and 16.** Condition means reproduce the run 14
archive (bids within 1–2.4 EUR, `act_share` to three decimals, ordering intact),
as finding 13 says they should while per-seed trajectories do not.

---

### 15 — the pivotal-frequency ladder

**Why:** run 14b established that `sb02b` solves the bertrand equilibrium and
leaves the pivotal one as noise — but in `sb02b` the regime that failed is also
the only regime that was **rare** (13 % of hours), so "pivotal" and "rare" are
perfectly confounded. This breaks them apart.

`p1`–`p7` are `sb02b`'s fleet (5 × 500 MW), market, fuel prices and seeds with
**only `demand_df` changed**, so the pivotal share runs 13 → 25 → 37 → 50 → 62
→ 75 → 88 %. Generated by `example_02_single_bid/make_demand_ladder.py`: an
affine compression toward the pivotal band's midpoint, `alpha` bisected per
target share.

**What is held fixed, and it is more than it looks.** The transform preserves
every hour's rank, so Pearson *and* Spearman against `p1` are both `1.000000`.
And because `learning_strategies.py:126-133` min-max scales residual load by
that series' own min and max, `alpha` **cancels**: the scaled observation is
identical across all seven rungs to 1e-4 (the CSV's `%.1f` rounding). The price
forecast is a flat 50.0 everywhere and the marginal-cost slot is fixed, so the
entire *exogenous* observation is bit-identical. What moves is where the regime
boundary falls inside it — 7000 MW at scaled 0.868 in `p1`, 0.231 in `p7`.
`p1` is `alpha = 1`, i.e. `p1` **is** `sb02b`, and reproduces it to the decimal.
...
- data: `runs/data/15-pivotal-ladder/`, img: `runs/img/15/`
- tarball: `eom_6275962.tar.gz` (701 MB — both sweeps, 21 trials)
- 7 cases × 3 seeds, 21/21 `COMPLETED`, 190 frames, 7600 critic updates

| case | pivotal | clearing price | profit/learner | reward | exploit. pivotal | bertrand |
|---|---:|---:|---:|---:|---:|---:|
| `p1` | 13 % | 52.73 | 234 | +0.002 | 2698 (0.34×) | 424 (**0.08×**) |
| `p3` | 38 % | 62.39 | 2827 | +0.053 | 659 (0.50×) | 1871 (0.21×) |
| `p5` | 63 % | 63.67 | 3809 | +0.074 | 841 (0.23×) | 5200 (0.56×) |
| `p7` | 88 % | 72.26 | 7415 | +0.146 | **260 (0.04×)** | 6313 (0.67×) |

**Reading — the two exploitability columns cross, and that is the result.** The
same regime converges when it is common and stays exploitable when it is rare,
in *both* directions. What a shared critic fails to fit is whatever is rare, not
...
- **Never read this fleet with a median over units.** From `p3` up the
  ...
  and the median of that is −100 EUR, which looks like total collapse and is
  not. The first pass at these results made exactly that mistake. Per-unit
  views only (`--only regime-heatmap`, which is one row per agent).
- Exploitability is **not monotone** in `k`. `p2`'s pivotal *rises* 5.85× off an
  anomalously low ep1; not chased down. The crossing is the finding, not the
  individual rungs.
- One unit of scaled observation is 3436 MW at `p1` and 352 MW at `p7`, so
  discrimination near the boundary is ten times finer at the top. That moves
  with frequency **by construction** (rank is preserved), so it is one variable
  seen twice rather than two confounded — but it is why the ladder cannot
  separate "more pivotal hours" from "a sharper boundary".
- 3 seeds per rung. `pp_6` is the marked-up unit in every seed and every rung,
  which is more consistent than the seed count can explain; not investigated.

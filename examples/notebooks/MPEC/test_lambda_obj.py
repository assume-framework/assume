"""
Test: Use lambda_ (strong-duality price) in the objective instead of lambda_hat.
Keep hat-KKT for complementarity/dispatch, but price the leader's revenue
using lambda_ which is constrained by the duality gap penalty.

Tight variable bounds for fast LP relaxation.
"""
...
def solve_lambda_obj(opt_gen, k_max, time_limit=600, big_w=10000, big_M=3500,
                     dual_bound=3500):
    ...
    # === OBJECTIVE: use lambda_ (NOT lambda_hat) for revenue ===
    def primary_obj(model):
        return sum(
            model.lambda_[t] * model.g[opt_gen, t]  # KEY CHANGE: lambda_ instead of lambda_hat
...
if __name__ == '__main__':
    ...
    for opt_gen, k_max, desc in configs:
        print(f'\n{"=" * 70}')
        ...
        t0 = time.time()
        res = solve_lambda_obj(opt_gen, k_max, time_limit=600, big_M=3500, dual_bound=3500)
        ...
        if res:
            ...
            print(f'\nTime: {elapsed:.1f}s')
            print(f'MCP (lambda_, obj):  {mcp.min():.2f} to {mcp.max():.2f}, mean={mcp.mean():.2f}')
            print(f'MCP_hat (hat-KKT):   {mcp_hat.min():.2f} to {mcp_hat.max():.2f}, mean={mcp_hat.mean():.2f}')
            ...
            print(f'Profit (lambda_): {profit:.0f} EUR/day')
        else:
